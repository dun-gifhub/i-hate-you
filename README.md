...
> **Trên Render gói free**, chỉ dùng được **Cách A (Brevo)** — Render chặn hết các cổng SMTP (25, 465, 587) trên gói free từ 26/9/2025, nên Gmail/SMTP thường sẽ báo "connection timeout". Gmail/SMTP (Cách B) chỉ dùng được khi chạy trên máy cá nhân hoặc gói Render trả phí.

### Cách A — Brevo (dùng được trên Render gói free)

1. Đăng ký miễn phí tại `https://www.brevo.com` (300 email/ngày miễn phí).
2. Vào **Settings → SMTP & API → API Keys**, tạo một API key mới.
3. Vào **Settings → Senders**, xác minh một địa chỉ email dùng để gửi (không cần tên miền riêng, xác minh bằng mã gửi qua email đó).
4. Điền vào `.env` (hoặc biến môi trường trên Render):

```
EMAIL_PROVIDER=brevo
BREVO_API_KEY=xkeysib-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
BREVO_SENDER_EMAIL=thuvienlop@gmail.com
MAIL_FROM_NAME=Thư viện lớp
```

### Cách B — Gmail (chỉ dùng trên máy cá nhân hoặc Render gói trả phí)
